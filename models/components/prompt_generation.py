...
from .utils import (DepthWiseConvBlock, DownChannelBlock, MaxPool2dSamePadding, CUDA_ADAPOOL2d)
from .semantic_extraction import (CrossAttentionBlock)
from models.sam2.sam2_utils import MLP


class SemanticPromptProjector(nn.Module):
    def __init__(self, image_in_channels: List[int], out_channels: int, num_stages: int = 2, start_level: int = 0,
                 epsilon: float = 1e-4, apply_bn_for_resampling: bool = True, conv_bn_act_pattern: bool = False,
                 num_heads: int = 8, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 has_mlp=False, padding: int = 1, pool_kernel: int = 4, pool_stride: int = 4) -> None:
        super().__init__()
        self.bifpn = BiFPN(
            in_channels=image_in_channels, out_channels=out_channels, num_stages=num_stages, start_level=start_level,
            epsilon=epsilon, apply_bn_for_resampling=apply_bn_for_resampling, conv_bn_act_pattern=conv_bn_act_pattern
        )
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.bn2 = nn.BatchNorm2d(out_channels)

        self.cross_att = CrossAttentionBlock(
            out_channels, num_heads=num_heads, stem_channel=out_channels,
            qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop, attn_drop=attn_drop,
            drop_path=drop_path, has_mlp=has_mlp
        )

        self.conv = nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=3, padding=padding)

        self.conv_dense = nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=3, padding=padding)
        # Use Default GPU
        self.adapool_sparse = CUDA_ADAPOOL2d(kernel_size=pool_kernel, beta=(1, 1), stride=pool_stride, return_mask=True,
                                             device='cuda')
        self.mlp_spase = MLP(out_channels // 4, out_channels // 2, out_channels, 3)

    def forward(self, img_features, cs_feature, pos_em):
        img_feature_f = self.bifpn(img_features)

        img_feature_f = self.bn1(img_feature_f)
        cs_feature = self.bn2(cs_feature)

        feature = self.cross_att(img_feature_f, cs_feature)
        feature = feature + pos_em

        feature = self.conv(feature)

        dense = self.conv_dense(torch.cat([img_feature_f, feature], dim=1))

        spase = self.adapool_sparse(feature)
        spase = self.mlp_spase(spase)
        return spase, dense


# Copyright (c) OpenMMLab. All rights reserved.
# Modified from https://github.com/open-mmlab/mmdetection/blob/main/projects/EfficientDet/efficientdet/bifpn.py
